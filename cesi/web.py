...
def control():
    if request.method == 'POST':
        ...
        test = ldap.bind_user(username, password) #Берёт полученные из формы данные и сверяет смотрит есть ли они в active directory
        if test is None or password == '':
            ...
            session['usertype'] = 0
            add_log = open(get_activity_log(), "a")
            add_log.write("%s - %s logged in.\n"%( datetime.now().ctime(), session['user_id'] ))
            return jsonify(status = "success")
...
# Dashboard
@app.route('/')
###############Проверка учётной записи и к какой группе она принадлежит
@ldap.login_required
@ldap.group_required(groups=['Administrators'])
###############
def showMain():
# get user type
...
def showNode(node_name):
    if session.get('logged_in'):
        ...
        add_log = open(get_activity_log(), "a")
        add_log.write("%s - %s viewed node %s .\n"%( datetime.now().ctime(), session['user_id'], node_name ))
...
def readlog(node_name, process_name):
    if session.get('logged_in'):
        if session['usertype'] == 0 or session['usertype'] == 1 or session['usertype'] == 2:
            node_config = get_config().getNodeConfig(node_name)
            node = Node(node_config)
            log = node.connection.supervisor.tailProcessStdoutLog(process_name, 0, 8192)[0]
            add_log = open(get_activity_log(), "a")
